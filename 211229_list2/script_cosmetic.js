...
function buildElement(product){
    ...
    <div class='product-description'>${product.description}</div>`//innerHTML로 태그를 바로 써서 입력가능.
    return item
}
...
//     oldProductList.push(...productList)
//     // 상품 정보에 대한 배열로부터 웹화면에 보여줄 DOM 객체로 이루어진 배열로 변환하기
...
const sortBtn=document.getElementById('price-btn')


console.log(oldProductList)
let state=true
function sortCosmetic(){
    state=!state
    while(nowItems.length!=0){
        nowItems.pop()
    }
    console.log(searchItems)
    const sortItems=[]
    sortItems.push(...searchItems)
    if(state){
        sortItems.sort((a,b)=>a.price-b.price)
        console.log(sortItems)
        nowItems.push(...sortItems)
        const productsRefined = sortItems.map(buildElement)
        ...
        nowItems.push(...sortItems)
        const productsRefined = searchItems.map(buildElement)
        while(rootDiv.hasChildNodes()){
            rootDiv.removeChild(rootDiv.firstChild)
        }
        productsRefined.forEach(displayProduct)
...
function searchItem(){
    ...
    if(!searchInput.value){
        ...
        productsRefined.forEach(displayProduct)
        searchItems.push(...oldProductList)
        return;
    }
    console.log("검색해볼께요")
    while(rootDiv.hasChildNodes()){
...
productList.push(...products)
searchItems.push(...productList)
oldProductList.push(...productList)
nowItems.push(...productList)
const show=products.map(buildElement)
show.forEach(displayProduct)